...
- `BuildCtx` is defined in `quartz/ctx.ts`. It consists of
  - `argv`: The command line arguments passed to the Quartz [[./build|build]] command
  - `cfg`: The full Quartz [[./configuration|configuration]]
  - `allSlugs`: a list of all the valid content slugs (see [[./paths|paths]] for more information on what a slug is)
...
A good example of a transformer plugin that borrows from the `remark` and `rehype` ecosystems is the [[./Latex|Latex]] plugin:
...
- Your component should use `getQuartzComponents` to declare a list of `QuartzComponents` that it uses to construct the page. See the page on [[./creating components|creating components]] for more information.
