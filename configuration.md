...
- `pageTitle`: title of the site. This is also used when generating the [[./RSS Feed|RSS Feed]] for your site.
- `pageTitleSuffix`: a string added to the end of the page title. This only applies to the browser tab title, not the title shown at the top of the page.
- `enableSPA`: whether to enable [[./SPA Routing|SPA Routing]] on your site.
- `enablePopovers`: whether to enable [[./popover previews|popover previews]] on your site.
...
- `locale`: used for [[./i18n|i18n]] and date formatting
...
- `ignorePatterns`: a list of [glob](<https://en.wikipedia.org/wiki/Glob_(programming)>) patterns that Quartz should ignore and not search through when looking for files inside the `content` folder. See [[./private pages|private pages]] for more details.
...
![[./docs/images/quartz transform pipeline.png|quartz transform pipeline.png]]
...
You should take care to add the plugin to the right entry corresponding to its plugin type. For example, to add the [[./ExplicitPublish|ExplicitPublish]] plugin (a [[tags/plugin/filter|Filter]]), you would add the following line:
...
For example, the [[./Latex|Latex]] plugin allows you to pass in a field specifying the `renderEngine` to choose between Katex and MathJax.
